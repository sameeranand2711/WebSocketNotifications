...
public sealed class NotificationProducerEndpointTests
{
    ...
    private static readonly string[] CombinedSubscriptions =
        ["group:operators", "feed:match-42", "tenant:abc:event:score.changed"];

    [Theory]
    [InlineData("/api/notifications/users", "users", "user:user-1", "users")]
    [InlineData("/api/notifications/subscriptions", "subscriptions", "group:operators", "subscriptions")]
        ...
        string expectedDimension)
    {
        ...
        {
            [requestProperty] = new[] { TargetFor(expectedDimension) },
            ["payload"] = new { score = 7 },
        };
        var response = await app.GetTestClient().PostAsJsonAsync(path, request);
    ...
    public async Task CombinedEndpoint_PublishesAnyRoutingCombinationAndExplicitKey()
    {
        ...
        Assert.Equal(CombinedSubscriptions, notification.Subscriptions);
        Assert.Equal("order:42", notification.Key);
        Assert.Equal(expiresAt, notification.ExpiresAt);
    }

    [Fact]
    public async Task UsersEndpoint_WhenUsersAreEmpty_ReturnsValidationProblemWithoutPublishing()
    {
        var publisher = new RecordingNotificationPublisher();
        await using var app = await StartApplicationAsync(publisher);
        ...
        var response = await app.GetTestClient().PostAsJsonAsync(
            ...
            new { users = Array.Empty<string>(), payload = new { value = 1 } });
    ...
    public async Task CombinedEndpoint_WhenPayloadIsMissing_ReturnsValidationProblemWithoutPublishing()
    {
        ...
        var response = await app.GetTestClient().PostAsJsonAsync(
            "/api/notifications",
            new { subscriptions = CombinedSubscriptions });
    ...
    public async Task Swagger_ExposesUiAndDocumentsEveryNotificationEndpoint()
    {
        ...
        var userProperties = schemas.GetProperty("UserNotificationRequest").GetProperty("properties");
        var subscriptionProperties = schemas.GetProperty("SubscriptionNotificationRequest").GetProperty("properties");
        ...
        Assert.True(paths.TryGetProperty("/api/notifications/subscriptions", out _));
        Assert.True(userProperties.TryGetProperty("users", out _));
        Assert.True(subscriptionProperties.TryGetProperty("subscriptions", out _));
    }
    ...
    private static void AssertDimension(QueuedNotification notification, string dimension)
    {
        ...
        Assert.Equal(dimension == "subscriptions" ? [TargetFor(dimension)] : [], notification.Subscriptions);
    }

...
public sealed class ProtocolProcessorTests
{
    ...
    public async Task ProcessAsync_WhenSubscribeIsAuthorized_AddsSubscriptionAndConfirms()
    {
        ...
        await processor.ProcessAsync(
            "connection-1",
            "user-1",
            Json("""{"type":"subscribe","requestId":"r1","kind":"group","value":"operators"}"""),
            outgoing,
        ...
        Assert.Equal("user-1", Assert.Single(authorizer.Contexts).UserId);
    ...
    public async Task ProcessAsync_WhenSubscribeIsDenied_DoesNotMutateStateAndReturnsError()
    {
        ...
        await processor.ProcessAsync(
            "connection-1",
            "user-1",
            Json("""{"type":"subscribe","requestId":"r2","kind":"feed","value":"private"}"""),
            outgoing,
            CancellationToken.None);
    ...
    public async Task ProcessAsync_WhenUnsubscribing_RemovesSubscriptionWithoutAuthorization()
    {
        ...
        await processor.ProcessAsync(
            "connection-1",
            "user-1",
            Json("""{"type":"unsubscribe","requestId":"r3","kind":"eventType","value":"score.changed"}"""),
            outgoing,
            CancellationToken.None);
    ...
    [Fact]
    public async Task ProcessAsync_WhenClientAttemptsDirectUserSubscription_RejectsCommand()
    {
        ...
        await processor.ProcessAsync(
            "connection-1",
            "user-1",
            Json("""{"type":"subscribe","requestId":"r4","kind":"user","value":"user-2"}"""),
            outgoing,
            CancellationToken.None);

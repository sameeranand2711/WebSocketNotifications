...
The repository-level `scripts/run-live-e2e.ps1` imports the same reusable client code for a one-host Kafka-to-WebSocket check. `scripts/run-multi-host-e2e.ps1` uses that client implementation to verify the shared-group failure mode, independent Kafka fan-out across two hosts, continuity after one host stops, broker stop/restart recovery, and no replay after restart.
